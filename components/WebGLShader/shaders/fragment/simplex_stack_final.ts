import { noiseUtils } from "@/components/WebGLShader/shaders/utils/noiseUtils";
import { simplex_noise } from "@/components/WebGLShader/shaders/utils/simplexNoise";
import { CreateFragmentShader, FragmentShaderUniforms } from "@/components/WebGLShader/shaders/types";
...
const createFragmentShader: CreateFragmentShader = (options) => {
  ...
  const shader = /* glsl */ `
    ...
    float noise(float x) {
      ...
      float sum = 0.0;
      sum += simplex_noise(vec2(x * (L / 1.00) + F * u_time2, u_time * S * 1.00)) * 0.85;
      sum += simplex_noise(vec2(x * (L / 1.30) + F * u_time2, u_time * S * 1.26)) * 1.15;
      sum += simplex_noise(vec2(x * (L / 1.86) + F * u_time2, u_time * S * 1.09)) * 0.60;
      sum += simplex_noise(vec2(x * (L / 3.25) + F * u_time2, u_time * S * 0.89)) * 0.40;
      return sum;
    }
